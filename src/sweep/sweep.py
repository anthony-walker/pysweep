...
def sweep(arr0,gargs,swargs,dType=np.dtype('float32'),filename ="results",exid=[]):
    ...
    MGST = int(np.ceil(TSO*(time_steps+1)/MOSS))    #THIS ASSUMES THAT time_steps > MOSS
    time_steps = int(np.ceil((MGST*MOSS+1)/TSO)) #Updating time steps
    ...
    for GST in range(1,2,1):#MGST):
        ...
        # comm.Barrier()  #Barrier after read
        # #Bridge Step
        # Bridge(sarr,xarr,yarr,XR,YR,(xtr,ytr),bridge_sets,wb+1,pargs) #THis modifies shared array
        # comm.Barrier()
        # #Getting next points for the local array
        # larr = np.copy(sarr[RR])
