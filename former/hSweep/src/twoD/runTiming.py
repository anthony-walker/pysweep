...
#%%
for p in eq:
    prog = op.join(tbinpath, p)
    prog += " "
    eqs = [k for k in tstrng if p.upper() in k.upper()]
    eqspec = op.join(ttestpath, eqs[0])
    eqspec += " "
    for sc in schemes:
        ...
        for t in tpb:
            for n in nX:
                ...
                for g in gpus:
                    ...
                    runMPICUDA(prog, nproc, sc, eqspec, mpiopt=mpiarg, eqopt=exargs, outdir=trspath)

        tfile = op.join(trspath, timeTitle)
