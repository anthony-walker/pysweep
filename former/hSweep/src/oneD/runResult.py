...
if __name__ == "__main__":
    ...
    if rn:
        ex = op.join(obinpath, kj.lower())
        km = [k for k in os.listdir(otestpath) if kj.lower() in k]
        eqf = op.join(otestpath, km[0])
        cout = runMPICUDA(ex, 8, sch, eqf + " ", outdir=orspath, eqopt=extra)
