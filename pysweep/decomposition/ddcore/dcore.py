...
from ddcore import block, decomp, sgs, functions, cpu
...
def cpu_core(sarr,total_cpu_block,shared_shape,OPS,BS,GRB,TSO,gargs):
    ...
    DecompObj = functions.DecompCPU(0,[(x+OPS,y+OPS) for x,y in np.ndindex((BS[0],BS[1]))],TSO-1,cwrite=swb,cread=total_cpu_block)
    sgs.carr = decomp.create_shared_pool_array(sarr[total_cpu_block].shape)
    return DecompObj,blocks

def gpu_core(blocks,BS,OPS,gargs,GRB,TSO):
