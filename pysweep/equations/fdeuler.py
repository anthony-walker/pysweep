...
def step(state,iidx,arrayTimeIndex,globalTimeStep):
    ...
    for idx,idy in iidx:
        fluxx = dtdx*(dudx(state[arrayTimeIndex],idx,idy)-dudx(state[arrayTimeIndex],idx-1,idy))
        fluxy = dtdy*(dudy(state[arrayTimeIndex],idx,idy)-dudy(state[arrayTimeIndex],idx,idy-1))
        state[arrayTimeIndex+1,:,idx,idy] = state[arrayTimeIndex,:,idx,idy]-fluxy-fluxx
    return state

def dudx(state,idx,idy):
    """This function finds the x derivative."""
    return -state[:,idx-1,idy]/12+7*state[:,idx,idy]/12+7*state[:,idx+1,idy]/12-state[:,idx+2,idy]/12

def dudy(state,idx,idy):
    """This function find the y derivative."""
    return -state[:,idx,idy-1]/12+7*state[:,idx,idy]/12+7*state[:,idx,idy+1]/12-state[:,idx,idy+2]/12
