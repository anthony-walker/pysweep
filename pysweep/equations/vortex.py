...
def steady_vortex(cvics,npx,npy,times=(0,),M_o=None,x0=0,y0=0):
    ...
    X = L
    Y = L
    u0 = M_inf*np.cos(alpha)
    v0 = M_inf*np.sin(alpha)
    a = np.sqrt(gamma*R_gas*T_inf)
    #Creating grid
    state = np.zeros(np.shape(times)+(4,npx,npy),dtype=np.float64) #Initialization of state
    for i,t in enumerate(times):
        xpts = np.linspace(-X,X,npx,dtype=np.float64)-x0-u0*t
        ypts = np.linspace(-Y,Y,npy,dtype=np.float64)-y0-v0*t
        xgrid,ygrid = np.meshgrid(xpts,ypts,sparse=False,indexing='ij')
        f = -1/(2*sigma*sigma)*((xgrid/r_c)**2+(ygrid/r_c)**2)
        Omega = beta*np.exp(f)
        du = -(ygrid)*Omega/r_c
        dv = (xgrid)*Omega/r_c
        ...
        state[i,1] = rho*u
        state[i,2] = rho*v
        state[i,3] = P/(gamma-1)+0.5*rho*(u*u+v*v)

    return state

def create_steady_data(cvics,npx,npy, times=(0,), x0=0, y0=0, filepath = "./vortex/",filename = "vortex",fdb=True):
    ...
    """
    #Args tuple
    state = steady_vortex(cvics,npx,npy, times, x0=x0, y0=y0)
    #Making directories
    if filepath == "./vortex/":
    ...
    #Creating state data
    data[:,rid,:,:] = state[:,rid,:,:]
    data[:,uxid,:,:] = state[:,uxid,:,:]
    data[:,vyid,:,:] =state[:,vyid,:,:]
    data[:,eid,:,:] =state[:,eid,:,:]
    return state
